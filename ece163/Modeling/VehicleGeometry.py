...
class VehicleGeometry():
	...
	def getNewPoints(self, x, y, z, yaw, pitch, roll):
		...
		# Compute the transpose of the DCM (R_b^v)
		dcm_transpose = MatrixMath.transpose(Rotations.euler2DCM(yaw, pitch, roll))

		# Rotate points using the transpose of the DCM
		rotated_points = []
		for point in newPoints:
			ned_point = [[point[0]], [point[1]], [point[2]]]  # Convert to column vector
			rotated_point = MatrixMath.multiply(dcm_transpose, ned_point)  # Apply rotation
			rotated_points.append([rotated_point[0][0], rotated_point[1][0], rotated_point[2][0]])  # Flatten

		# Translate the rotated points in the NED inertial frame
		translated_points = MatrixMath.offset(rotated_points, x, y, z)

		# Convert the translated points from NED to ENU
		enu_points = [Rotations.ned2enu(point) for point in translated_points]
